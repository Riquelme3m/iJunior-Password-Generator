...
const Password = ({ password }) => {
    async function copyPassword(password) {
        if (password && password !== 'P4$5W0rD!' && password !== 'Please select at least one character type.') {
            try {
    ...
    return (
        <div className="container-password">
            <p id="password" className={getPasswordClassName()}>{password}</p>
            <button id="copy" onClick={() => copyPassword(password)}>
